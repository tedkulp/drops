...
func TestViewIssueReadsTheWholePageAtOneRevision(t *testing.T) {
	...
	if view.Parent == nil || view.Parent.ParentID != parent.ID {
		t.Fatalf("view parent = %#v, want %s", view.Parent, parent.ID)
	}
	if len(view.Dependencies) != 1 || view.Dependencies[0].ToID != parent.ID {
	...
	if len(up.Children) != 1 || up.Children[0].ChildID != child.ID {
		t.Fatalf("children = %#v", up.Children)
	}
	if len(up.Dependents) != 1 || up.Dependents[0].FromID != child.ID {
